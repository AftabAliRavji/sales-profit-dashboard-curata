...
def clear_day_state():
    ...
    This ensures a clean slate before importing new bulk data.
    """
    ...
    with tabs[0]:
        ...
        for day_index in range(int(days)):
            ...
            with st.expander(
                ...
                # Order loop begins here — continues in Part 4
                # Continue order loop from Part 3
            ...
            if file_name.endswith(".json"):
                ...
                if "json_preview" in st.session_state:
                    ...
                    for date_key, day_data in st.session_state["json_preview"].items():
                        ...
                        if not isinstance(day_data.get("orders", []), list):
                            validation_messages.append(
                ...
                if st.button("Import JSON data"):
                    try:
                        data = st.session_state.get("json_preview", None)
                        if not data:
                        ...
                        # Count summary
                        total_days = len(data)
                        total_orders = sum(len(day.get("orders", [])) for day in data.values())

                        # Run import
                        parse_json_bulk(file_obj)

                        st.success(f"✅ Imported {total_days} days and {total_orders} orders.")
    ...
    with tabs[3]:
        ...
        if df.empty:
            ...
            if not monthly.empty:
                monthly["Profit %"] = (
                    (monthly["Profit ($)"] - monthly["Ad Spend ($)"])
                    / monthly["Sales ($)"]
                    * 100
                ).fillna(0)
...
def parse_json_bulk(uploaded_file):
    ...
    if not isinstance(data, dict):
        st.error("JSON must contain a dictionary of dates.")
        return
    ...
    for date_str, day_data in data.items():
        try:
            ...
            st.warning(f"Skipping invalid date: {date_str}")
            continue

        # Determine day index relative to start_date
        start_date = st.session_state.get("start_date", date.today())
        day_index = (day_date - start_date).days
        if day_index < 0:
            st.warning(f"Skipping {date_str}: before start_date.")
            continue

        # Ensure days count is large enough
        if day_index + 1 > st.session_state["days"]:
            st.session_state["days"] = day_index + 1

        # Apply ad spend
        if "ad_spend" in day_data:
            st.session_state[f"ad_spend_day_{day_index}"] = float(day_data["ad_spend"])

        # Apply visitors
        if "visitors" in day_data:
            st.session_state["visitors_per_day"] = int(day_data["visitors"])

        # Apply orders
        orders = day_data.get("orders", [])
        if isinstance(orders, list):
            st.session_state[f"orders_day_{day_index}"] = len(orders)
            for i, order in enumerate(orders, start=1):
                st.session_state[f"day_{day_index}_order_{i}_sales"] = float(order.get("sales", 0))
                st.session_state[f"day_{day_index}_order_{i}_profit"] = float(order.get("profit", 0))

    st.success("JSON bulk import complete. Rerunning...")
    st.rerun()


def parse_csv_bulk(uploaded_file):
    try:
    ...
    for _, row in df.iterrows():
        ...
        st.session_state[f"ad_spend_day_{day_index}"] = float(row["ad_spend"])
        st.session_state["visitors_per_day"] = int(row["visitors"])

        order_index = int(row["order_index"])
        st.session_state[f"orders_day_{day_index}"] = max(
            st.session_state.get(f"orders_day_{day_index}", 1),
            order_index,
        )

        st.session_state[f"day_{day_index}_order_{order_index}_sales"] = float(row["sales"])
        st.session_state[f"day_{day_index}_order_{order_index}_profit"] = float(row["profit"])

    st.success("CSV bulk import complete. Rerunning...")
    st.rerun()
...
"""
        ...
        if uploaded_bulk is not None:
            ...
            if file_name.endswith(".json"):
                if st.button("Import JSON data"):
                    parse_json_bulk(uploaded_bulk)

            elif file_name.endswith(".csv"):
                if st.button("Import CSV data"):
                    parse_csv_bulk(uploaded_bulk)

            else:
                st.warning("Unsupported file type. Please upload a .json or .csv file.")
    ...
    # ---------------------- Tab 8: Summary Charts (BAR CHART VERSION) ---------------------- #
    with tabs[7]:
